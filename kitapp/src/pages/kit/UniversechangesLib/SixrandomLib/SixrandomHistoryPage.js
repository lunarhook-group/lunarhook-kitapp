...
class SixrandomHistoryPage extends React.Component {
  ...
  */
  ...
  _starRow(rowData) {
    StorageModule.load({ key: 'sixrandom', id: rowData.id }).then(ret => {
      var Jobj = JSON.parse(ret);
      Jobj.star = true == Jobj.star ? false : true
      ret = JSON.stringify(Jobj)
      StorageModule.save({ key: 'sixrandom', id: rowData.id, data: ret })
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
  ...
  _deletehistory(rowData) {
    StorageModule.remove({ key: 'sixrandom', id: rowData.id });
    this.refreshlist();
  }

  async convert(){
    //StorageModule.clearMapForKey('sixrandom');
    await StorageModule.getAllDataForKey('sixrandom').then(ids => {
      //alert(ids)
      for (i = 0; i < ids.length; i++) {
          //console.log("convertJsonfinal",ids[i]);
      }
    }).catch(err => {console.warn(err.message);})
    convertHistoryNameArray = new Array()
    await StorageModule.getAllDataForKey('user').then(ids => {
      console.log("converttests",ids)
      for (i = 0; i < ids.length; i++) {
          var randArray = ids[i];
          console.log("convert",randArray)
          ...
          console.log("convertJson",Jstr)
          convertHistoryNameArray.push(Jstr)
      }
    }).catch(err=>{
      console.log(err);
    });
    for(i = 0; i < convertHistoryNameArray.length; i++)
    {
      var obj = JSON.parse(convertHistoryNameArray[i])
      console.log("convertJsonsave",obj,obj.date)
      await StorageModule.save({key:"sixrandom",id:obj.date,data:convertHistoryNameArray[i]})
      //await StorageModule.remove({key:"user",id:convertHistoryNameArray[i].date})
    }
    await StorageModule.clearMapForKey('user');
  ...
  refreshlist() {
    ...
    StorageModule.getAllDataForKey('sixrandom').then(ids => {
      //alert(ids)
      for (i = 0; i < ids.length; i++) {
        try {
          ...
          console.log(error)
          //StorageModule.remove({ key: 'sixrandom', id: randArray[7] });
    ...
    let content = (<View>
      ...
      <SwipeListView
        ...
        renderItem={(data, rowMap) => (<TouchableHighlight
          ...
          onPress={() => navigate('SixrandomMainPage', data.item.url)}
