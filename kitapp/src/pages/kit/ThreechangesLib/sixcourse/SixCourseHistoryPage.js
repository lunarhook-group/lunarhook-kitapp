...
class SixCourseHistoryPage extends React.Component {
  ...
  */
  ...
  _starRow(rowData){
    StorageModule.load({key:'sixcourse',id:rowData.id}).then(ret=>{
      console.log(ret)
      ret[6]=true==ret[6]?false:true
      StorageModule.save({key:'sixcourse',id:rowData.id,data:ret})
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
  ...
  _deletehistory(rowData)
  {
    console.log(rowData.id)
      StorageModule.remove({key:'sixcourse',id:rowData.id});
      this.refreshlist();
  }
  ...
  refreshlist()
  {
    ...
    StorageModule.getAllDataForKey('sixcourse').then(ids => {
    ...
    let content = (<View>
            <SwipeListView
                    ...
                    renderItem={ (data, rowMap) => (<TouchableHighlight
                      onPress={ () => navigate('SixCourseMainPage',data.item.url)}
