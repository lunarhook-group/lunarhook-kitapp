...
import { Card, Button, Modal, Toast, WhiteSpace, WingBlank,Provider, Portal } from '@ant-design/react-native';
...
class qimenHistoryPage extends React.Component {
  ...
  */
  ...
  _starRow(rowData){
    StorageModule.load({key:'qimen',id:rowData.id}).then(ret=>{
      console.log(ret)
      ret[4]=true==ret[4]?false:true
      StorageModule.save({key:'qimen',id:rowData.id,data:ret})
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
  ...
  _deletehistory(rowData)
  {
    console.log(rowData.id)
      StorageModule.remove({key:'qimen',id:rowData.id});
      this.refreshlist();
  }
  ...
  refreshlist()
  {
    ...
    StorageModule.getAllDataForKey('qimen').then(ids => {
    ...
    let content = (<View>
            <SwipeListView
                    ...
                    renderItem={ (data, rowMap) => (<TouchableHighlight
                      onPress={ () => navigate('qimenMainPage',data.item.url)}
