...
class EightrandomHistoryPage extends React.Component {
  ...
  */
  ...
  _starRow(rowData){
    StorageModule.load({key:'name',id:rowData.id}).then(ret=>{
      console.log(ret)
      ret[5]=true==ret[5]?false:true
      StorageModule.save({key:'name',id:rowData.id,data:ret})
      this.refreshlist();
    })
  }
  _updateRow(rowData) {
...
<View>
            <SwipeListView
                    ...
                    renderItem={ (data, rowMap) => (<TouchableHighlight
                      onPress={ () => navigate('EightrandomMainPage',data.item.url)}
                    ...
                    >
                      ...
                      <Card style={{ width: width-20 }}>
                          ...
                          <Card.Body>
                            <View >
                              <Text  style={{ marginLeft: 16 }}>{data.item.name}:{data.item.tip}</Text>
                            </View>
                          </Card.Body>
                          <Card.Footer content={data.item.birth} />
                        </Card>
                      </View>
                    </TouchableHighlight>
                  )}
