    ...
    if (false == switchtype) {
      return (
        <View className="demo-list-item">
          <Text>{leaptype}</Text>
          <Switch name={leaptype} checked={("常年" == switchleap) ? true : false} onChange={(value) => {
            this.setState({ switchleap: value.detail.value, leaptype: value.detail.value == false ? "常年" : "闰月" })
          }} />
        </View>
      )
      ...
      <View className='contain'>
        <View >
          <Form>
            ...
            <Switch  checked={"男" == this.state.selectedValue ? true : false} onChange={(value) => this.setState({ switchstate: value.detail.value, selectedValue: false == value.detail.value ? "女" : "男" })} />
            </View>
            <View className="demo-list-item">
            <Text>{this.state.datatype}</Text>
            <Switch checked={"公历" == this.state.datatype ? true : false} onChange={(value) => this.setState({ switchtype: value.detail.value, datatype: value.detail.value == false ? "农历" : "公历" })} />
            </View>
        ...
        <View>
          <Button onClick={() => this.bazipaipan()} type='secondary'>八字测评</Button>
        </View>
      </View>
    )
  }
